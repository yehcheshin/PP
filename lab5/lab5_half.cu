...
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    //("tid: (%d,%d) : blockid (%d,%d) blockDim(%d,%d) gridDim.x(%d) \n",threadIdx.x,threadIdx.y,blockIdx.x,blockIdx.y,blockDim.x,blockDim.y,gridDim.x);
    ...
    //int y = tid;
       //if (threadIdx.x < 1 && threadIdx.y<1) printf("(%d,%d) %d  %d\n",blockIdx.x,blockIdx.y,blockDim.x,blockDim.y);
    //printf("blockdim.x %d  blockDim.y =%d blockIdx.x=%d blocjIdx.y=%d (%d %d)\n",blockDim.x,blockDim.y,blockIdx.x,blockIdx.y,threadIdx.x,threadIdx.y);
    //printf("%d %d %d %d %d\n",gridDim.x,blockDim.x,blockDim.y,threadIdx.x,threadIdx.y);
    ...
    for(int y = blockIdx.x * blockDim.y+threadIdx.y; y<height;y+=gridDim.x * blockDim.y){
        for (int x = threadIdx.x ;x < width;x+= blockDim.x) {
            ...
            /* Z axis of mask */
                ...
                int h =H;
                int w =W ;
                for (int v = -yBound; v <= yBound; ++v,h++) {
                    for (int u = -xBound ; u <= xBound; ++u,w++) {
                        // for (int v = -yBound; v <= yBound; ++v) {
                //     for (int u = -xBound ; u <= xBound; ++u) {
                        if (bound_check(x + u, 0, width) && bound_check(y + v, 0, height)) {
                            // pixel_ker[(y+v)%S][(x+u)%S+2] = s[channels * (width * (y + v) + (x + u)) + 2];
                            // pixel_ker[(y+v)%S][(x+u)%S+1] = s[channels * (width * (y + v) + (x + u)) + 1];
                            // pixel_ker[(y+v)%S][(x+u)%S+0] = s[channels * (width * (y + v) + (x + u)) + 0];
                            ...
                            val[i][2] = __hadd( val[i][2], __float2half(__hmul(R , mask[i][u + xBound][v + yBound])));
                            val[i][1] = __hadd(val[i][1],__float2half(__hmul(G , mask[i][u + xBound][v + yBound])));
                            val[i][0] = __hadd( val[i][0],__float2half(__hmul(B , mask[i][u + xBound][v + yBound])));
        ...
        __half totalB = 0.;
        for (int i = 0; i < Z; ++i) {
            totalR = __hadd(totalR,__float2half( __hmul(val[i][2],val[i][2])));
            totalG = __hadd(totalG,__float2half(__hmul(val[i][1],val[i][1])));
            totalB = __hadd(totalB,__float2half(__hmul(val[i][0],val[i][0])));
        }
        // totalR = (sqrt(totalR) / SCALE);
        // totalG = (sqrt(totalG) / SCALE);
        // totalB = (sqrt(totalB) / SCALE);
        totalR = __hdiv(hsqrt(totalR) , __int2half_rn(SCALE));
        totalG = __hdiv(hsqrt(totalG),__int2half_rn(SCALE));
        totalB = __hdiv(hsqrt(totalB),__int2half_rn(SCALE));
        unsigned char cR = __half2float(totalR > __float2half(255.)) ? __half2float(255.) : __half2float(totalR);
        unsigned char cG = __half2float(totalG > __float2half(255.)) ? __half2float(255.) : __half2float(totalG);
        unsigned char cB = __half2float(totalB > __float2half(255.)) ? __half2float(255.) : __half2float(totalB);
...
int main(int argc, char **argv) {
    ...
    // dim3 num_blocks(width/num_threads.x+1,height/num_threads.y+1);
    const int num_blocks=(height/num_threads.x*num_threads.y +1);
    //printf("%d",height/num_threads.y*num_threads.x +1);//3265個block
    //printf("%d",width/32);154
    //printf("%d",num_blocks);
    // launch cuda kernel
