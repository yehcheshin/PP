...
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    ...
    __shared__ unsigned char pixel_ker[1028][5][3] ; 
        ...
        for (int x = threadIdx.x ;x < width;x+= blockDim.x) {
            ...
            for (int v = -yBound; v <= yBound; ++v) {
                if ( bound_check(y + v, 0, height)){
                        ...
                        pixel_ker[x%1028][(y+v)%5][2] = s[channels * (width * (y+v) + (x)) + 2];
                        pixel_ker[x%1028][(y+v)%5][1] = s[channels * (width * (y+v) + (x)) + 1];
                        pixel_ker[x%1028][(y+v)%5][0] = s[channels * (width * (y+v) + (x)) + 0];
                       
                        if(x%1024==-1){
                            for(int i =1;i<=4;i++){
                                pixel_ker[(x+i)%1028][(y+v)%5][2] = s[channels * (width * (y+v) + (x+i)) + 2];
                                pixel_ker[(x+i)%1028][(y+v)%5][1] = s[channels * (width * (y+v) + (x+i)) + 1];
                                pixel_ker[(x+i)%1028][(y+v)%5][0] = s[channels * (width * (y+v) + (x+i)) + 0];
                            }
                        }
            ...
            for (int v = -yBound; v <= yBound; ++v) {
                for (int u = -xBound; u <= xBound; ++u) {
                    if (bound_check(x + u, 0, width) && bound_check(y + v, 0, height)) {
                        ...
                        const unsigned char R =   pixel_ker[(x+u)%1028][(y+v)%5][2];
                        const unsigned char G =   pixel_ker[(x+u)%1028][(y+v)%5][1];
                        const unsigned char B =   pixel_ker[(x+u)%1028][(y+v)%5][0];
